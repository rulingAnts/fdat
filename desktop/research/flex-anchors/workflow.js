...
return { report, stats: { investigations: sweep.length, findings: totalFindings, verified: verified.length, confirmed: verified.filter(v => v.verdict && v.verdict.verdict === 'confirmed').length, refuted: verified.filter(v => v.verdict && v.verdict.verdict === 'refuted').length, gaps: gapResults.length } }
