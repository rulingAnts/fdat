export const meta = {
  ...
  description: 'Research where FDAT data can be anchored/linked in the FLEx (LCM) data model, with adversarial verification',
  phases: [
    { title: 'Sweep', detail: 'eight investigators over liblcm, FieldWorks, flexbridge, flexlibs/FLExTools and GitHub docs' },
    { title: 'Verify', detail: 'each key claim re-checked against its source by one or two skeptics' },
...
const results = await pipeline(
  ...
  async (res, inv) => {
    ...
    const verified = await parallel(claims.map((f, i) => async () => {
      const lenses = f.critical ? ['source', 'behaviour'] : ['source']
      const votes = (await parallel(lenses.map(lens => () =>
        agent(verifyPrompt(f, lens), { label: `verify:${inv.key}#${i + 1}:${lens}`, phase: 'Verify', schema: VERDICT })))).filter(Boolean)
      return { ...f, votes }
    }))
    return { key: inv.key, summary: res.summary, openQuestions: res.openQuestions, findings: verified.filter(Boolean) }
  }
)
...
${digest(sweep)}`, { label: 'critic', phase: 'Critique', schema: GAPS, effort: 'high' })
...
const gapResults = await pipeline(
  gaps,
  (g, _o, i) => agent(`${CONTEXT}\nGAP INVESTIGATION #${i + 1}\nQUESTION: ${g.question}\nWHY IT MATTERS: ${g.why}\nWHERE TO LOOK: ${g.whereToLook}\nAnswer the question with cited findings.`, { label: `gap:${i + 1}`, phase: 'Gap sweep', schema: FINDINGS, effort: 'high' }),
  async (res, g, i) => {
    if (!res) return null
    const claims = res.findings.slice(0, 8)
    const verified = await parallel(claims.map((f, j) => async () => {
      const lenses = f.critical ? ['source', 'behaviour'] : ['source']
      const votes = (await parallel(lenses.map(lens => () =>
        agent(verifyPrompt(f, lens), { label: `verify:gap${i + 1}#${j + 1}:${lens}`, phase: 'Gap sweep', schema: VERDICT })))).filter(Boolean)
      return { ...f, votes }
    }))
    return { key: `gap: ${g.question}`, summary: res.summary, openQuestions: res.openQuestions, findings: verified.filter(Boolean) }
  }
)
...
log(`total findings: ${all.reduce((n, r) => n + r.findings.length, 0)}, confirmed ${confirmedCount}, refuted ${refutedCount}`)
...
return { report, digest: digest(all), stats: { confirmed: confirmedCount, refuted: refutedCount } }